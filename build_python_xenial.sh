...
PYTHON_LOCAL=/opt/python380-at12

echo "Python local = $PYTHON_LOCAL"

echo "PWD = ${PWD}"

./configure CC="/opt/at12.0/bin/gcc" CXX="/opt/at12.0/bin/g++" --with-lto --enable-optimizations --enable-ipv6 --enable-loadable-sqlite-extensions --prefix=${PYTHON_LOCAL} --exec-prefix=${PYTHON_LOCAL}
